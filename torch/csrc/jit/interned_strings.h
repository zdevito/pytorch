...
#define FORALL_BASE_SYMBOLS(_) \
  _(prim) \
  _(aten) \
  _(onnx) \
  _(attr) \
  _(scope) \
  _(none) /* returned when a string does not have a namespace */
...
enum class _keys : unique_t {
    #define DEFINE_KEY(s) s,
    FORALL_BASE_SYMBOLS(DEFINE_KEY)
    #undef DEFINE_KEY
...
FORALL_BASE_SYMBOLS(DEFINE_SYMBOL)
}
#undef DEFINE_SYMBOL
