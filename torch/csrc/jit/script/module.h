...
struct Module {
  ...
  Method& create_method_from_trace(
      std::string name,
  ...
  Method* find_method(const std::string& name) const {
    ...
    if (Function* fn = class_cu().find_function(name)) {
      return lower_first_class_method(fn);
    } else if(Function* fn = lowered_methods().find_function(name)) {
      return lift_lowered_method(fn);
    }
 ...
 private:
  ...
  Method* lower_first_class_method(Function* fn) const;
  Method* lift_lowered_method(Function* fn) const;
...
std::shared_ptr<Graph> propagate_shapes(Graph& graph,
